...
public class Client extends Application {
	...
	public static void soundSetup() {
		...
		Media addWatchlistWav = new Media(
				getClass().getClassLoader().getResource("final_project/addWatchlist.wav").toString());
		watchAddSound = new MediaPlayer(addWatchlistWav);

		Media remWatchlistWav = new Media(
				getClass().getClassLoader().getResource("final_project/remWatchlist.wav").toString());
	...
	private static Scene loginScene(Stage screen) {
		...
		greetingBpane.setStyle("-fx-background-image: url(\"/final_project/lightSky.png\");");
	...
	private static Scene auctionSceneDisplay(Stage screen) {
		...
		itemSelections.getStylesheets().add("final_project/choicebox.css");
			...
			public void run() {
				while (!updatesComplete) {
					...
					while (!bidHistory.isEmpty()) {
						...
						Platform.runLater(() -> {
							bidTab.getItems().add(biddingDisplay.replace(user, "you"));
						});
					}
			...
			private void sellAlert() {
				...
				Platform.runLater(() -> {
					...
					if (!(selling.contains(user))) {
						...
						purchaseTab.getItems().add(selling.replace(user, "you"));
		...
		Thread buttons = new Thread(new Runnable() {
			@Override
			public void run() {
		...
		Thread watchlist = new Thread(new Runnable() {
			@Override
			public void run() {
				while (!updatesComplete) {
					synchronized (wLock) {
			...
			private void updateWatchedItem(String nameKey, AuctionItem i) {
				if (nameKey.contentEquals(i.title)) {
					...
					Platform.runLater(() -> {
						String currBiddingString = "$" + decimalForm.format(i.currBidding);

						if (i.currBidding == 0.00) {
							currBiddingString = "none";
						}

						biddingLabel.setText(
								"Minimum Bid Price: $" + decimalForm.format(i.minimumPrice) + "  Buy Now Price: $" +
										decimalForm.format(i.buyNow) + "  Highest Bid: " + currBiddingString +
		...
		watchlist.setName("threadUpdatingWatch");

		startThreads(notifications, buttons, watchlist);
		...
		add.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				String newItem = itemSelections.getValue();

				if (!watchNames.contains(newItem)) {
					...
					for (AuctionItem i : auctionItems) {
						...
						if (i.title.contentEquals(newItem)) {
					...
					VBox entireItemVBox = finalVbox(userWatchlist, newItem, title, itemDesc,
							itemDetailsVBox, itemsSeparator);

					watchNames.add(newItem);
					watchedItems.add(newItem);
					wlPairs.add(new Pair<String, VBox>(newItem, entireItemVBox));
				} else {
					wlAddErr.setText("The item - " + newItem + " - is already on your watchlist.");
		...
		remove.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				synchronized (wLock) {
					...
					if (watchNames.remove(itemToRemove)
						...
						removeFromWatchlist(wlAddErr, bidError, userWatchlist);
					} else {
			...
			private void removeFromWatchlist(Label error, Label bidError, ListView<VBox> wlView) {
		...
		eCodeScene.getStylesheets().add("final_project/myStyle.css");
		return eCodeScene;
	}
	...
	private static void auctionDisplay(Label watchlist, ListView<VBox> watchlistView, Label tabs,
		...
		display.setStyle("-fx-background-image: url(\"/final_project/lightSky.png\");");
	...
	private static void tabFill(TabPane updates, ListView<String> purchased, ListView<String> bids) {
		bids.getStylesheets().add("final_project/list.css");
		purchased.getStylesheets().add("final_project/list.css");
	...
	private static ListView<> watchingListView() {
